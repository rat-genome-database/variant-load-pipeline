 ...
 */
public class PolyphenLoader2 extends VariantProcessingBase {
...
//        XmlBeanFactory bf=new XmlBeanFactory(new FileSystemResource("properties/AppConfigure.xml"));
    ...
    public void run(int sampleId) throws Exception {
        ...
        List<String> infos = loadInfos(sampleId);

        String resultFileName = getWorkDir() + "/" + sampleId + ".polyphen";
...
//N
            ...
            int varPos = Integer.parseInt(cols[6].trim()); // pos
        ...
        this.getLogWriter().write(rowsInserted+" polyphen results were loaded into db for sample "+sampleId+"\n");
        this.getLogWriter().write(linesProcessed+" polyphen results were processed for sample "+sampleId+"\n\n");
    ...
    // load info file for polyphen input
    // lines: variant_id|variant_transcript_id|locus_name|protein_acc_id|pos|ref_aa|var_aa|strand|transcript_rgd_id
    List<String> loadInfos(int sampleId) throws Exception {

        List<String> infos = new ArrayList<String>();

        String infoFileName = getWorkDir() + "/Sample" + sampleId + ".PolyPhenInput.info";
