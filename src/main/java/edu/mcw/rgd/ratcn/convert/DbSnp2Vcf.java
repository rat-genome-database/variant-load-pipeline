...
public class DbSnp2Vcf {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DbSnp2Vcf converter = new DbSnp2Vcf();
        for( int i=0; i<mapKeys.size(); i++ ) {
            converter.run(mapKeys.get(i), sources.get(i), outputFiles.get(i));
        }

        System.out.println("---OK---");
    }

    void run(int mapKey, String source, String outputFile) throws Exception {

        String ucscAssemblyId = MapManager.getInstance().getMap(mapKey).getUcscAssemblyId().toUpperCase();

        System.out.println("DBSNP2VCF started");
        ...
        writer.write("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t"+source.toUpperCase()+"_"+ucscAssemblyId+"\n");
...
*/
    ...
    void writeVcfLine(Record r, BufferedWriter writer) throws IOException {
        ...
        writer.write("\t0/1:"+refCount+","+varCount+":9");
